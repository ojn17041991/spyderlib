...
var __pausable: bool = true setget set_pausable, get_pausable
...
func _process(delta):
	if __pausable and Input.is_action_just_pressed("input_pause"):
		...
		if __paused:
			if allow_freeze:
				$SpyPause_Freezer.pause_mode = Node.PAUSE_MODE_STOP
			get_tree().paused = true
			emit_signal("paused")
		else:
			if allow_freeze:
				$SpyPause_Freezer.pause_mode = Node.PAUSE_MODE_PROCESS
