...
func _ready():
	...
	add_child(__patcher)
	add_child(__deleter)
	__getter.connect("request_completed", _get_request_completed)
	__poster.connect("request_completed", _post_request_completed)
	__patcher.connect("request_completed", _patch_request_completed)
	__deleter.connect("request_completed", _delete_request_completed)

func GET(_endpoint):
	__getter.request(base_address + _endpoint, [], HTTPClient.METHOD_GET)
...
func _get_request_completed(_result, _response_code, _headers, _body):
	...
	if _result == HTTPRequest.RESULT_SUCCESS and _response_code >= 200 and _response_code < 300:
		...
		_dict = JSON.parse_string(_response_str)
	emit_signal("get_callback", _response_code, _dict)

func POST(_endpoint, _payload):
	__poster.request(base_address + _endpoint, ["Content-Type: application/json"], HTTPClient.METHOD_POST, _payload)
...
func PATCH(_endpoint, _payload):
	__patcher.request(base_address + _endpoint, [], HTTPClient.METHOD_PATCH, _payload)
...
func DELETE(_endpoint):
	__patcher.request(base_address + _endpoint, [], HTTPClient.METHOD_DELETE)
